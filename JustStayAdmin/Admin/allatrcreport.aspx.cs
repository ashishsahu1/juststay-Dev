...
{
    public partial class allatrcreport : BL.BasePage
    {
        ...
        private void BindGrid()
        {
            ...
            try
            {
                ...
                List<GetAllATRC_Report> customerlist = reportClient.GetAllATRCReport(fromdate, todate, Convert.ToString(textsearch.Text.Trim())).ToList();
                if (customerlist == null) return;
                totalrecord.Text = "Total Records: " + customerlist.Count.ToString();

                grdatrc.DataSource = customerlist;
                grdatrc.DataBind();
                reportClient.Close();
            }
            catch (Exception ex)
        ...
        private void ExportGridToExcel()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(txtfromdate.Value) && !string.IsNullOrEmpty(txttodate.Value))
                    Response.AddHeader("content-disposition", "attachment;filename=AllATRCReport" + Convert.ToString(txtfromdate.Value) + "to" + Convert.ToString(txttodate.Value) + ".xls");
                else
                    Response.AddHeader("content-disposition", "attachment;filename=AllATRCReport.xls");
                Response.Charset = "";
        ...
        private void ExportGridToPDF()
        {
            try
            {
                ...
                Response.Buffer = true;
                if (!string.IsNullOrEmpty(txtfromdate.Value) && !string.IsNullOrEmpty(txttodate.Value))
                    Response.AddHeader("content-disposition", "attachment;filename=AllATRCReport" + Convert.ToString(txtfromdate.Value) + "to" + Convert.ToString(txttodate.Value) + ".pdf");
                else
                    Response.AddHeader("content-disposition", "attachment;filename=AllATRCReport.pdf");
                Response.Charset = "";
        ...
        protected void grdatrc_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                grdatrc.PageIndex = e.NewPageIndex;
                BindGrid();
